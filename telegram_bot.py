...
async def send_prompt_to_intermediary(prompt):
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{INTERMEDIARY_URL}/predict", json={"prompt": prompt})
    ...
    if telegram_update.message and telegram_update.message.text:
        ...
        if get_nonce(chat_id) is None:  # User has no valid nonce, meaning they're verified or need to connect
            if message.lower().startswith("/generate_image"):
                ...
                if success:
                    # Here you might want to handle the response from the intermediary, like sending the image or a success message
                    if 'image' in response:
                        # TODO: Implement sending images via Telegram API
                        await application.bot.send_message(chat_id=chat_id, text="Image generated! Here's your robo-hippo: " + response['image'])
                    else:
