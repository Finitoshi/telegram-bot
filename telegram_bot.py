...
nonce_collection = db['nonces']  # New collection for storing nonces securely
...
def generate_nonce(user_id):
    nonce = os.urandom(32).hex()  # Generate 32 bytes of random data, then convert to hex
    ...
    logger.info(f"Generated nonce for user {user_id}. Expires at {expiry}")
    return nonce
...
def get_nonce(user_id):
    ...
    if nonce_data and nonce_data['expiry'] > datetime.utcnow():
        ...
        logger.info(f"Nonce expired or not found for user {user_id}.")
        return None
